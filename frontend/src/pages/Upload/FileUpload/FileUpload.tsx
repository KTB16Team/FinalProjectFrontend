...
const ALLOWED_FILE_TYPES = ['audio/mpeg', 'audio/wav', 'image/png', 'image/jpeg']; // 허용 파일 형식
...
export default function FileUpload() {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (selectedFile) {
      if (!ALLOWED_FILE_TYPES.includes(selectedFile.type)) {
        showModal(
          '지원하지 않는 파일 형식입니다. 음성 파일(mp3, wav) 또는 이미지 파일(png, jpeg)만 업로드 가능합니다.',
          () => {
          }
        );
        return;
  ...
  const handleConfirmUpload = async () => {
    ...
    try {
      ...
      // Prefix 결정 로직
      let prefix: "IMAGE" | "AUDIO" | "TEXT" | undefined;
      if (['png', 'jpg', 'jpeg'].includes(extension!)) {
        prefix = 'IMAGE';
      } else if (['mp3', 'wav', 'ogg', 'acc', 'flac', 'm4a'].includes(extension!)) {
        prefix = 'AUDIO';
    ...
    <div className="relative">
      ...
      <Body>
        ...
        <div>
          ...
          <input
            type="file"
            accept="audio/mpeg, audio/wav, image/png, image/jpeg" // 파일 타입 지정
